...
Para servir contenido Web, especialmente si este es dinámico, debemos contar con un servidor Web. Hay varios existentes, y uno de los más utilizados es Apache HTTP Server. Otra alternativa puede ser NGINX.

Si deseamos podemos crear nuestro propio servidor Web personalizado, para lo que podemos utilizar frameworks como Flask, si necesitamos una aplicación web dinámica.

Si solo necesitamos una aplicacion que permita entregar únicamente contenido web estático (html, css, js, imágenes, ...) podríamos incluso usar el [servidor pre-programado](https://github.com/daoc-icam/python?tab=readme-ov-file#pre-programado) de base en Python:

```python
python3 -m http.server 8080
```

> Y habremos creado un servidor web, que reciba clientes en el puerto 8080, y que entregue el contenido del directorio actual
...
Revisemos nuestra primera transacción Http. Abra su navegador, abra las Developer tools, y ponga en la línea de direcciones la URL: `http://fiipi.webs.vc/index.html`. En el panel de las Dev.Tools seleccione, arriba, "Network", y abajo seleccione "index.html". A la derecha seleccione "Headers" y ahí podrá ver los encabezados de la transacción Http. Primero verá información general, y hacia abajo verá información específica de la Response y la Request:
